...
interval_document_authors <- function(start_date, end_date, keyword = NULL, search_id = NULL, granularity = "DAY"){
  ...
  if(is.null(keyword)){
    url <- paste0("https://api.meltwater.com/insights/v1/intervals/count/document_authors?search_id=", search_id, "&start_date=",
                  start_date, "T00%3A00%3A00Z&end_date=", end_date, "T23%3A59%3A59Z&granularity=", granularity)
  }else{
    url <- paste0("https://api.meltwater.com/insights/v1/intervals/count/document_authors?keyword=", keyword, "&start_date=",
                  start_date, "T00%3A00%3A00Z&end_date=", end_date, "T23%3A59%3A59Z&granularity=", granularity)
  }
